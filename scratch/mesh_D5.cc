...
using namespace ns3;
  ...
  */
  ...
  std::string out_file_for_convergence_times = "./Richard_output_logs/TRY-OUT-convergence_grid_topology_10x1_tvws_regulation_compliant_mesh.csv";
 ...
 */
  ...
  m_randomStart (0.1),
  m_totalTime (20), //Richard: simulation time in seconds, I think?
  /*
   * ******************************************************************************
...
PrintReceivedPacket (Ptr<Socket> socket, Ptr<Packet> packet, Address senderAddress)
{
  ...
  else
  {
	  ...
	  table[index].neighbourChannels = stringToList(s); //s is info packet content received
	  table[index].commonChannels = intersectionCalc(stringToList(s), table[index].glsdResponse );
...
*/
  ...
  */
   ...
   * */
  Ptr<Node> node = nodes.Get(9); //will cause an error if node count is less than 4
...
/*START try of setDown, setUp */
     ...
     uint32_t ipv4ifIndex = 1;
      Simulator::Schedule (Seconds (0),&Ipv4::SetDown,ipv4, ipv4ifIndex);
      Simulator::Schedule (Seconds (0),&Ipv4::SetUp,ipv4, ipv4ifIndex);
...
// Config::ConnectWithoutContext("/NodeList/[0]/DeviceList/*/$ns3::WifiPhy::MonitorSnifferRxTracedCallback", MakeCallback (&PhyTrace));
  ...
  Simulator::Run ();
  //the "." before the file path moves one step out of directory of execution
  flowmon->SerializeToXmlFile (("./folder_for_measurements/mesh_D5_m_hwmp.xml"), false, false); //my version ~Richard
...
void MeshTest::MonitorSniffRx2 (std::string context, Ptr<const Packet> packet, uint16_t channelFreqMhz, uint16_t channelNumber, WifiTxVector txVector, MpduInfo aMpdu, SignalNoiseDbm signalNoise)
  {
	...
	/*
	    	 	...
	    	 	std::list<int>::iterator first_item_node_i_commonChannels = table[index-1].commonChannels.begin();
	    	 	...
	    	 	if (index > 0 && index < m_xSize*m_ySize && !table[index].commonChannels.empty())
	    	 	{
	    	 		 ...
	    	 		 */
	    	 		if (table[index].connected==true &&
	    	 				table[index].channelSwitched==false
