...
using namespace ns3;
  ...
  */
  ...
  std::string out_file_for_convergence_times = "./Richard_output_logs/convergence_grid_topology_2x2_tvws_regulation_compliant_mesh.csv";
 ...
 */
	 std::cout<<"nodeId, deviceId, currentChannel, activeChannels, candidateChannel, invokedBeaconDetectionXtimes, connected? "<< std::endl; //header row
	  for(std::vector<ScanStatus>::iterator i = table.begin(); i != table.end(); ++i)
	  {
	      ...
	      std::cout<<"\t\t"<<i->candidateChannel<<", \t\t"<<i->invokedBeaconDetectionXtimes<<", \t\t\t\t"<<i->connected<<std::endl;
	  }//end for
 ...
 */
  m_xSize (2),
  m_ySize (2), //total number of nodes = m_xSize * m_ySize
  ...
  int nsinks = m_xSize*m_ySize-1;
    ...
    for (int i = 0; i < nsinks; i++) //Richard.
    {
     std::cout<<" Here"<<std::endl;
  	...
  	temp.Start (Seconds (m_totalTime)); //Richard: set it equal or higher than simulation time to disable it
...
*/
  ...
  */
   ...
   * */
  Ptr<Node> node = nodes.Get(3); //will cause an error if node count is less than 4
 ...
 */
	...
	//std::thread t1([&]{ /* The '&' is a fix to the compilation error: "in lambda function ..... this was not captured for this lambda function" */
	      std::list<int> listOfchannels ={40, 36}; //36, 44, 40, 48, et al. This is the ordering that works when candidate channel=40
	      std::list <int> :: iterator it;
	    	 ...
	    	 // BeaconDetected = &node1Associated; //BeaconDetected local variable pointing to the address of "detected" global variable
	          for(it = listOfchannels.begin(); it != listOfchannels.end(); ++it)
	          {
	             ...
	             //Simulator::Schedule (Seconds ( Simulator::Now ().GetSeconds ()+delay), &setChannel, channelSettingString,  channelNum, index); //call setChannel() at scheduled time. This syntax is for when function being called is not member of the MeshTest class.
	              ...
	              delay=delay+channelListenDuration; //using channelListenDuration to have a constant increment
	           ...
	           // bool & detected2 = ::node5Associated; //attempting to assing value by reference instead of by value, but no lucj
	    	   ...
	    	   /// std::string  deviceId=context.substr( 23,1 );
	    	    ...
	    	    std::string deviceId = indexExtractor(context, "DeviceList/", "/Phy" ); //new approach, robust enough
	    	 	...
	    	 	if (index - (m_xSize+1) < 0)
	    	 	{
					...
					else if (*findIter != channelNumber)
					{
						table[index].chan.push_back(channelNumber);
	    	 	...
	    	 	else // i.e. if (index - (m_xSize+1) => 0), factor in the fact that beacon can come from multiple sources in a not so linear manner
	    	 	{
	    	 		/*
...
//void setChannel (std::string channelSetter, UintegerValue chann)
  void MeshTest::setChannel (std::string channelSetter, int chann, int nodeIndex)
  {
	 // int 	var = 0;
	  ...
	  std::string i =std::to_string(nodeIndex);
...
main (int argc, char *argv[])
{
  ...
  std::string nodeId = indexExtractor("/NodeList/100/DeviceList/1/Phy/MonitorSnifferTx", "/NodeList/", "/DeviceList" );
  std::string deviceId = indexExtractor("/NodeList/99/DeviceList/4/Phy/MonitorSnifferTx", "DeviceList/", "/Phy" );
