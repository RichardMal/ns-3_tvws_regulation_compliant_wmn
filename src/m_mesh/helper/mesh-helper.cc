 ...
 */
#include "m_mesh-helper.h" //Richard
...
#include "ns3/node-list.h" //Richard
#include "ns3/hwmp-rtable.h" //Richard
#include "ns3/internet-module.h" //Richard
...
void
MeshHelper::PrintRoutingTableAllAt (Time printTime, Ptr<OutputStreamWrapper> stream, Time::Unit unit)
{
  for (uint32_t i = 0; i < NodeList::GetNNodes (); i++)
    {
      Ptr<Node> node = NodeList::GetNode (i);
     Simulator::Schedule (printTime, &MeshHelper::Print, node, stream, unit);

    }
}

void
MeshHelper::Print (Ptr<Node> node, Ptr<OutputStreamWrapper> stream, Time::Unit unit)
{
	/*
	Richard: attempting to replicate routing table print out in ipv4-routing-helper.cc
...
*/
  ...
  Ptr<dot11s::HwmpRtable> table = CreateObject<dot11s::HwmpRtable> (); //Richard

  Ptr<Node> meshNode = node->GetObject<Node>();
  Ptr<NetDevice> dev = meshNode->GetDevice(0);
  ...
  //table = hwmpProt->GetObject<dot11s::HwmpRtable>();

  /*
   * Richard: I need to assign table the value of hwmp instances on the node:(
   */
  //I've tried  but, no luck:(
  //table = meshDev->GetRoutingProtocol();
  //table= l2routingProtocol;

/*
  std::cout<<"In MeshHelper::Print: Node#? \n";
  std::cout<<"--Destination--"<<"--Retransmitter--"<<"--Interface--"<<"--Metric--"<<"--Expires--"<<"--Sequence #--\n";
  std::cout<<"--###########--"<<"--"<<table->LookupProactiveExpired().retransmitter<<"--";
  std::cout<<"--"<<table->LookupProactiveExpired().ifIndex<<"--"<<table->LookupProactiveExpired().metric <<"--";
  std::cout<<"--"<<table->LookupProactiveExpired().lifetime<<"--"<<table->LookupProactiveExpired().seqnum<<"--\n";
 */
