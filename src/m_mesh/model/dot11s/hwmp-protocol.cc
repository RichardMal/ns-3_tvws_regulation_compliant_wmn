...
//#include "hop-count-metric.h" //Richard
//#include "hop-count-metric.cc" //Richard:using this line as a crude fix to, "undefined reference to HopCountLinkMetricCalculator" error
//#include "etx-metric.h" //Richard
//#include "etx-metric.cc" //Richard
//#include "hop-count-metric-augmented.h" //Richard
//#include "hop-count-metric-augmented.cc" //Richard
//#include "etx-metric-augmented.h" //Richard
//#include "etx-metric-augmented.cc" //Richard
//#include "airtime-metric-augmented.h" // Richard
//#include "airtime-metric-augmented.cc" // Richard
...
NS_LOG_COMPONENT_DEFINE ("m_HwmpProtocol");
  ...
  if (result.retransmitter != Mac48Address::GetBroadcast ())
    {
      ...
      m_stats.txUnicast++;//Ptr<HopCountLinkMetricCalculator> metric = CreateObject <HopCountLinkMetricCalculator> ();
      //mac->SetLinkMetricCallback (MakeCallback (&HopCountLinkMetricCalculator::CalculateMetric, metric));
       ...
       * Richard
       */
      std::cout<<"in HwmpProtocol::ForwardUnicast(): current hop: "<<GetAddress()<<" source: "<<source
    		  <<" Destination: "<<destination<<" Retransmitter: "<<result.retransmitter<<" Interface: "<<result.ifIndex<<" Packet ID: "<< packet->GetUid() <<std::endl;
      return true;
    }
  if (sourceIface != GetMeshPoint ()->GetIfIndex ())
    {
  ...
  for (std::vector<Ptr<NetDevice> >::const_iterator i = interfaces.begin (); i != interfaces.end (); i++)
    {
      ...
      //Install hop-count link metric -Richard
/*
      Ptr<HopCountLinkMetricCalculator> metric = CreateObject <HopCountLinkMetricCalculator> ();
      mac->SetLinkMetricCallback (MakeCallback (&HopCountLinkMetricCalculator::CalculateMetric, metric));
*/
      //End hop-count-link metric -Richard

      //Install etx link metric -Richard
/*
      Ptr<EtxLinkMetricCalculator> metric = CreateObject <EtxLinkMetricCalculator> ();
      mac->SetLinkMetricCallback (MakeCallback (&EtxLinkMetricCalculator::CalculateMetric, metric));
*/
      //End etx link metric -Richard

      //Install augment hop-count link metric -Richard
/*
       Ptr<HopCountAugmentedLinkMetricCalculator> metric = CreateObject <HopCountAugmentedLinkMetricCalculator> ();
       mac->SetLinkMetricCallback (MakeCallback (&HopCountAugmentedLinkMetricCalculator::CalculateMetric, metric));
*/

       //End augmented  hop-count-link metric -Richard

      //Install augmented etx link metric -Richard
/*
       Ptr<EtxAugmentedLinkMetricCalculator> metric = CreateObject <EtxAugmentedLinkMetricCalculator> ();
      mac->SetLinkMetricCallback (MakeCallback (&EtxAugmentedLinkMetricCalculator::CalculateMetric, metric));
*/
       //End augmented etx link metric -Richard

      //install augmented airtime link metric -Richard
/*
      Ptr<AirtimeAugmentedLinkMetricCalculator> metric = CreateObject <AirtimeAugmentedLinkMetricCalculator> ();
      mac->SetLinkMetricCallback (MakeCallback (&AirtimeAugmentedLinkMetricCalculator::CalculateMetric, metric));
*/


      //end augmented airtime link metric -Richard
    }
  ...
  while (packet.pkt != 0)
    {
      ...
      /* Richard: when mesh root is set to broadcast (ff:ff:..:ff) only reactive mode works */
      /*
      std::cout<<"Richard: in hwmpProtocol::ReactivePathResolved(): packet source: "<<packet.src <<" Destination: "<<packet.dst<<" Interface: "<<result.ifIndex<<" Retransmitter: "<<result.retransmitter;
      std::cout<<" Protocol: "<<packet.protocol<<" Packet: "<<packet.pkt <<std::endl;
      */
    }
  ...
  while (packet.pkt != 0)
    {
      ...
      /* Richard: I hade not gotten ouput from here, which means, have not had any proactive routing happen */
      std::cout<<"Richard: in hwmpProtocol::ProactivePathResolved(): packet source: "<<packet.src <<" Destination: "<<packet.dst<<" Interface: "<<result.ifIndex<<" Retransmitter: "<<result.retransmitter;
      std::cout<<" Protocol: "<<packet.protocol<<" Packet: "<<packet.pkt <<std::endl;
