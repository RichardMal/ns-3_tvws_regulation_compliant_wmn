...
#include "etx-metric-augmented.h"
#include "ns3/wifi-phy.h"
#include "ns3/core-module.h" //Richard

namespace ns3 {
namespace dot11s {
NS_OBJECT_ENSURE_REGISTERED (EtxAugmentedLinkMetricCalculator);
...
TypeId
EtxAugmentedLinkMetricCalculator::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::dot11s::EtxAugmentedLinkMetricCalculator")
    .SetParent<Object> ()
    .SetGroupName ("Mesh")
    .AddConstructor<EtxAugmentedLinkMetricCalculator> ()
    .AddAttribute ( "TestLength",
                    "Rate should be estimated using test length.",
                    UintegerValue (1024),
                    MakeUintegerAccessor (
                      &EtxAugmentedLinkMetricCalculator::SetTestLength),
    ...
    .AddAttribute ( "Dot11MetricTid",
                    ...
                    UintegerValue (0),
                    MakeUintegerAccessor (
                      &EtxAugmentedLinkMetricCalculator::SetHeaderTid),
                    MakeUintegerChecker<uint8_t> (0)
                    )
  ;
  return tid;
}
EtxAugmentedLinkMetricCalculator::EtxAugmentedLinkMetricCalculator ()
{
}
void
EtxAugmentedLinkMetricCalculator::SetHeaderTid (uint8_t tid)
{
...
void
EtxAugmentedLinkMetricCalculator::SetTestLength (uint16_t testLength)
{
  m_testFrame = Create<Packet> (testLength + 6 /*Mesh header*/ + 36 /*802.11 header*/);
}
uint32_t
EtxAugmentedLinkMetricCalculator::CalculateMetric (Mac48Address peerAddress, Ptr<MeshWifiInterfaceMac> mac)
{
  ...
  /***************************************************
   * 1. For ETX, lets keep things simple by considering frame error rate to be equal to etx
   * 2. Instead of changing return turn of EtxAugmentedLinkMetricCalculator::CalculateMetric () from uint32_t
  ...
  //or
  //Richard: considering r to be the datarate -drawing inspiration from original airtime metric calculation
 ...
 // uint32_t r = (uint32_t)((double)(mac->GetWifiPhy ()->CalculateTxDuration (m_testFrame->GetSize (),txVector, mac->GetWifiPhy ()->GetFrequency())).GetMicroSeconds ());
  ...
  //return  (uint32_t)failAvg/r; //Richard. this compiles fine but, ...no better than etx
  //return  (uint32_t)failAvg*r; //Richard: try this? .. same output as previous line

  uint32_t beta = 0.1; //Richard: scaling constant, tryout values: 0.1, 0.5, 0.9
 //return priority;
 return (uint32_t)failAvg+(priority*beta); //prioritise links with higher PHY bit rate
